...
- **Modo Historia — Actos 5, 6, 7 y 8 (el tramo final)**: la campaña pasa de cuatro actos a ocho y se cierra. Cada acto trae mapa propio, mecánica propia, una cinemática firma y un salto de dificultad real (niveles 72 → 100, `version_tier` 2 → 5).
  ...
  - Contenido de rivales, mazos, perfiles de IA y recompensas en las migraciones `161`→`164`, **aplicadas a producción el 2026-09-10** (antes de desplegar el código: son filas nuevas, no tocan nada de los actos 1-4). Documentación por acto en [docs/story/acts](docs/story/acts/README.md) y diseño conjunto en [ACTS-5-8-MASTER-GUIDE.md](docs/story/acts/ACTS-5-8-MASTER-GUIDE.md).
  - **Curva de dificultad corregida** (migración `165`, también aplicada el 2026-09-10): las migraciones de contenido dejaron los mazos rivales con los stats pelados del catálogo —sin atributos propios, al contrario que los actos 1-4— y la curva de nivel solo suma +150 de ataque entre el nivel 72 y el 100, así que el primer duelo del Acto 5 salía **más blando que el primero del Acto 4**. Ahora cada duelo fija nivel, `version_tier` y atributos base: el ataque efectivo medio encadena 2700→3180 (Acto 5), 3000→3490 (6), 3520→3910 (7) y 3640→**4230** (8), justo por encima de la mejor fusión del jugador con mejora.
