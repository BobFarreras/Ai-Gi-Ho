...
import {
  readAdminStarterDeckDragData,
  writeAdminStarterDeckDragData,
} from "@/components/admin/internal/admin-starter-deck-dnd";
...
export function AdminStarterDeckPanel({ initialData }: IAdminStarterDeckPanelProps) {
  ...
  const selectedCard =
    (selectedSlotCardId ? cardById.get(selectedSlotCardId) ?? null : null) ??
    (editor.selectedCollectionCardId ? cardById.get(editor.selectedCollectionCardId) ?? null : null);
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="text-xs font-semibold uppercase tracking-wide text-slate-200">
            ...
            <select
              ...
              className="mt-1 block rounded-md border border-slate-600 bg-slate-900 p-2 text-xs text-slate-100"
          ...
          <button type="button" aria-label="Refrescar starter deck" className="rounded-md border border-cyan-500 px-3 py-2 text-xs font-bold uppercase text-cyan-200" onClick={() => void editor.onRefresh()} disabled={editor.isBusy}>
            Refrescar
          </button>
          <button type="button" aria-label="Activar o desactivar modo edición starter deck" className="rounded-md border border-slate-600 px-3 py-2 text-xs font-bold uppercase text-slate-100" onClick={() => editor.setIsEditMode(!editor.isEditMode)} disabled={editor.isBusy || !editor.data.template}>
            {editor.isEditMode ? "Salir de edición" : "Editar plantilla"}
          </button>
          <button
            type="button"
            aria-label="Aplicar carta seleccionada al slot activo"
            className="rounded-md border border-slate-600 px-3 py-2 text-xs font-bold uppercase text-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <button
            ...
            className="rounded-md border border-rose-600 px-3 py-2 text-xs font-bold uppercase text-rose-200 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <button type="button" aria-label="Guardar starter deck" className="rounded-md border border-emerald-500 px-3 py-2 text-xs font-bold uppercase text-emerald-200 disabled:cursor-not-allowed disabled:opacity-50" onClick={() => void editor.onSave()} disabled={!editor.isEditMode || editor.isBusy || !editor.data.template || !editor.canSave}>
            Guardar
          </button>
        </div>
        {editor.feedback ? <p className="mt-2 text-xs text-slate-200">{editor.feedback}</p> : null}
      </div>
      ...
      {editor.data.template ? (
        <div className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[320px_minmax(0,1fr)_320px]">
          ...
          <AdminStarterDeckDeckPanel
            ...
            onSelectSlot={editor.setSelectedSlotIndex}
          ...
          <AdminStarterDeckCollectionPanel
            ...
            isEditMode={editor.isEditMode}
            onSelectCard={editor.setSelectedCollectionCardId}
