...
function FusionPlaybackItem({ item, onDone }: { item: IFusionPlaybackItem; onDone: () => void }) {
  ...
  return (
    <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/70 backdrop-blur-[2px] flex items-center justify-center">
      {phase === "video" && videoSrc ? (
        <video autoPlay muted playsInline className="w-[min(72vw,980px)] rounded-2xl border border-cyan-400/45 shadow-[0_0_45px_rgba(6,182,212,0.35)]" onEnded={() => setPhase("summon")} onError={() => setPhase("summon")}>
