...
function renderBeam(source: IPoint, target: IPoint, color: { glow: string; core: string }, key: string, delay = 0) {
  ...
  const controlY = (source.y + target.y) / 2 - 38;
  ...
  return (
    <svg key={key} className="absolute inset-0 h-full w-full" viewBox="0 0 1000 1000" preserveAspectRatio="none">
      <motion.path d={path} fill="none" stroke={color.core} strokeWidth={8} strokeLinecap="round" initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: [0, 1], opacity: [0, 1, 0] }} transition={{ duration: 0.5, delay, ease: "easeOut" }} style={{ filter: color.glow }} />
    </svg>
  );
}
...
export function EffectTargetedOverlay({ events, playerAId }: IEffectTargetedOverlayProps) {
  ...
  useEffect(() => {
    const id = window.requestAnimationFrame(() => {
      ...
      const target = targetId ? findEntityPoint(overlayRef.current, targetId) : null;
  ...
  return (
    <div ref={overlayRef} className="pointer-events-none absolute inset-0 z-[390] overflow-hidden">
      ...
      {trapSignal && trapTargetPoint && trapSourcePoint && trapSignal.action === "NEGATE_ATTACK_AND_DESTROY_ATTACKER" ? renderBeam(trapSourcePoint, trapTargetPoint, trapDestroyColor, `${trapSignal.id}-destroy`, 0.46) : null}
      {statSignal && statSourcePoint ? (
        <motion.div initial={{ opacity: 0, scale: 0.72 }} animate={{ opacity: [0, 0.95, 0], scale: [0.72, 1.18, 0.95] }} transition={{ duration: 0.42, ease: "easeOut" }} className={statSignal.amount >= 0 ? "absolute h-16 w-16 rounded-full bg-yellow-300/65 blur-sm" : "absolute h-16 w-16 rounded-full bg-violet-300/65 blur-sm"} style={{ left: `${(statSourcePoint.x / 1000) * 100}%`, top: `${(statSourcePoint.y / 1000) * 100}%`, transform: "translate(-50%,-50%)" }} />
      ) : null}
      {statSignal && statSourcePoint ? statTargetPoints.map((targetPoint, index) => renderBeam(statSourcePoint, targetPoint, statColor, `${statSignal.id}-target-${index}`, 0.18 + index * 0.05)) : null}
    </div>
  );
}
