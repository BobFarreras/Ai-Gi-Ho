...
function ProgressItem({ label, value, icon, tone }: IProgressItemProps) {
  ...
  return (
    <div className={`flex items-center gap-2 rounded-sm border px-2 py-1 shadow-[inset_0_0_15px_rgba(0,0,0,0.2)] ${toneClass}`}>
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-sm border border-white/10 bg-black/30 sm:h-10 sm:w-10">
        <Icon className={`h-4 w-4 animate-pulse sm:h-5 sm:w-5 ${iconClass}`} />
      </div>
      <div className="flex flex-col text-left">
        <span className="font-mono text-[7px] uppercase tracking-widest text-white/60 sm:text-[8px]">{label}</span>
        <span className={`font-mono text-sm font-black leading-tight sm:text-base ${valueClass}`}>{value}</span>
      </div>
    </div>
  );
}
