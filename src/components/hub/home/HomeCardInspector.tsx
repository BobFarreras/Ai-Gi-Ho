...
export function HomeCardInspector({ selectedCard }: HomeCardInspectorProps) {
  return (
    <aside className="rounded-2xl border border-cyan-900/45 bg-[#030c16]/86 p-4 shadow-[0_0_24px_rgba(8,145,178,0.14)]">
      <h2 className="mb-3 text-sm font-black uppercase tracking-widest text-cyan-200">Detalle</h2>
      {selectedCard ? (
        <>
          <div className="mx-auto w-fit origin-top scale-[0.62]">
            <Card card={selectedCard} />
          </div>
          <p className="-mt-20 text-lg font-black uppercase text-cyan-100">{selectedCard.name}</p>
          <p className="mt-1 text-xs uppercase tracking-widest text-cyan-300/80">
            {selectedCard.type} · {selectedCard.faction}
          </p>
          <p className="mt-2 text-xs text-slate-300">{selectedCard.description}</p>
        </>
      ) : (
